...
def problem2b(rect, n, delta, win):
    ...
    bleft = rect.get_lower_left_corner()
    uright = rect.get_upper_right_corner()
    for k in range(n):
        point1 = rg.Point(bleft.x - (k)*delta, bleft.y + (k)*delta)
        point2 = rg.Point(uright.x + (k)*delta, uright.y - (k)*delta)
        newrect = rg.Rectangle(point1, point2)
